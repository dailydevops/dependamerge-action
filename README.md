...
| `command`                  | Merge Method with which the pull request is to be merged.<br/><br/>Command `squash` is the default command. All commits are squashed into one commit and merged into the target branch.<br />Command `merge` merges the pull request with the target branch, keeping the commit history.<br />Command `rebase` rebases the pull request onto the target branch if it is behind, maintaining individual commits without creating a merge commit. |    ❌    | `squash`                      | `squash`, `merge`, `rebase`      |
